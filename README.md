...
## Public-ready proof flow

Если вы готовите публичную публикацию или меняли bootstrap/config/docs surface, проходите proof всегда в одном порядке:

1. Этот `README.md`: проверьте, что quickstart всё ещё ведёт по path `новый Spreadsheet -> local .clasp.json -> первый scan`, без live binding и без reuse чужого production-container.
2. `docs/repository-sanitization.md`: подтвердите правило `no live bindings in Git` и границы tracked surface.
3. `docs/public-ready-checklist.md`: быстро пройдите checklist по tracked code/config/docs, placeholders, inventory и отдельной проверке `ImagesService`.
4. `powershell -ExecutionPolicy Bypass -File scripts/check-public-surface.ps1`: запустите automated audit и принимайте public-ready состояние только при `Public surface audit passed.`
