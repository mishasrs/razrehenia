...
- Есть неблокирующий metadata drift: `.planning/STATE.md` фиксирует фазу 01 как завершённую (`completed`, `3/3`), а таблица progress в `.planning/ROADMAP.md` всё ещё показывает Phase 1 как `2/3` и `In Progress`.
...
1. На чистом clone пройти `README.md` до `clasp push -f` и `clasp pull` с новым Spreadsheet и новым Apps Script container, затем убедиться, что после обновления таблицы появляется menu `Разрешения`.
2. Выполнить `Разрешения -> Указать папку с PDF` и `Разрешения -> Сканировать`, затем подтвердить появление листов `Разрешения`, `Logs`, `Витрина` и ожидаемых записей в `Logs`.
3. Для QR runtime выполнить `Разрешения -> Тест: 1 файл` или `diagnoseOne()` на реальном PDF с QR и убедиться, что нет ошибок вида `ImagesService is not defined`.
4. Если scan уходит в long-running сценарий, отдельно подтвердить создание installable trigger `continueScan`; если нужен Telegram, отдельно подтвердить `tgInitEmployeeBot()` и trigger `tgProcessEmployeeBotUpdates()`.

## Вывод

По состоянию tracked-файлов, документации и автоматического audit фаза 01 выглядит собранной и соответствует `BOOT-03`, а также документально закрывает `BOOT-01` и `BOOT-02`. Итоговый статус оставлен как `human_needed`, потому что ключевое утверждение goal про bootstrap другого maintainer ещё должно быть подтверждено одним узким ручным прогоном в реальном Google-контуре.
